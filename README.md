# 🚗 안녕하세요, 차량용 ECU 및 임베디드 소프트웨어 엔지니어 안해성입니다.

하드웨어와 소프트웨어의 긴밀한 상호작용을 깊이 이해하고, 차량용 시스템에서 가장 중요한 **안전성(Safety)**과 **신뢰성(Reliability)**을 갖춘 코드를 작성하는 것에 가치를 둡니다.
실제 차량 부품의 신뢰성 검증 환경을 경험하며 통신 및 하드웨어 제어 루틴의 최적화 역량을 키워왔습니다.

---

## ⚙️ Core Competencies
* **임베디드 최적화**: C/C++ 기반 객체의 불필요한 소멸자 오버헤드를 방지하기 위한 참조 전달(Pass-by-reference) 등 메모리 및 부하 최적화 고려 고려
* **차량용 및 산업용 통신**: CAN, LIN, SPI 등의 프로토콜을 활용한 레지스터 레벨의 디버깅 및 데이터 동기화
* **신뢰성 검증**: 차량용 도메인의 DV/PV 테스트 프로세스 이해 및 가혹 환경에서의 하드웨어 제어 신뢰성 검증 경험

---

## 🚀 Experience & Training
* **보쉬전장 (Bosch Transmission)** | Testing & Prototyping 팀 인턴 (6개월)
    * 차량용 부품 DV/PV 환경 시험 프로세스 수행 및 규격 평가
    * ECF(Engine Cooling Fan) PWM 제어 로직 검증 및 하드웨어 연동 테스트
* **Intel AI SW 가치확산 선도인력 양성 과정** | 900시간 수료
    * 지능형 시스템 및 고성능 임베디드 소프트웨어 개발 역량 내재화

---

## 🛠️ Tech Stacks

### 💻 Embedded Software
<p>
  <img src="https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=black"/>
  <img src="https://img.shields.io/badge/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white"/>
  <img src="https://img.shields.io/badge/Embedded C-00599C?style=flat-square&text=Embedded%20C"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
</p>

### 📡 Protocols & Hardware Interface
<p>
  <img src="https://img.shields.io/badge/CAN / LIN-000000?style=flat-square&text=CAN/LIN"/>
  <img src="https://img.shields.io/badge/SPI / I2C-🔥?style=flat-square&text=SPI/I2C"/>
  <img src="https://img.shields.io/badge/PWM Control-00599C?style=flat-square&text=PWM"/>
  <img src="https://img.shields.io/badge/STM32 / MCU-03234B?style=flat-square&logo=stmicroelectronics&logoColor=white"/>
</p>

### 🗄️ Databases & Tools
<p>
  <img src="https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=mariadb&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/DBML-⚙️?style=flat-square&text=DBML"/>
</p>

---

## 📂 Highlighted Projects

### 📟 모터 제어 시뮬레이션 및 ECU 연동 레포지토리 (가장 강조할 개인 작업물 링크)
* **설명**: 차량용 모터 제어(예: PWM 제어 알고리즘) 및 ECU 거동을 모사한 임베디드 소프트웨어 프로젝트
* **핵심 역량**: 타이머 인터럽트 기반 PWM 제어 최적화 및 타이트한 메모리 제약 조건 관리
* **Tech Stack**: C++, MCU, PWM Control

### 🏭 Smart Factory MES (제조실행시스템) 및 데이터 통합
* **설명**: CAN, LIN, OPC UA, Modbus TCP 등 이기종 프로토콜 간의 실시간 데이터 동기화를 구현한 시스템
* **핵심 역량**: DBML 기반의 체계적인 공정 테이블 구조 설계 및 다중 통신 환경에서의 데이터 무결성 보장
* **Tech Stack**: C++, Python, CAN, LIN, MariaDB

### ⠎ 솔레노이드 액추에이터 기반 실시간 점자 제어 시스템 (Graduation Project)
* **설명**: 18개의 솔레노이드 액추에이터를 동시 제어하며 가혹 구동 조건에서의 신뢰성을 확보한 하드웨어-소프트웨어 통합 프로젝트
* **핵심 역량**: 대용량 액추에이터 구동 시의 전력 분배 및 실시간 태스크 스케줄링 디버깅, 시스템 신뢰성 평가
* **Tech Stack**: Embedded C, MCU, Actuator Control

---

## 📈 GitHub Stats
<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=HaesungAhn0205&show_icons=true&theme=dark" alt="Haesung's github stats" />
</p>

📬 **Contact** : [내 이메일 주소] | [링크드인 주소]
